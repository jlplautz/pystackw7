...
def calcula_equilibrio_financeiro():
    gastos_essenciais = (
        Valores.objects.filter(data__month=datetime.now().month)
        .filter(tipo='S')
        .filter(categoria__essencial=True)
    )
    gastos_nao_essenciais = (
        Valores.objects.filter(data__month=datetime.now().month)
        .filter(tipo='S')
        .filter(categoria__essencial=False)
    )
    ...
    try:
        ...
        percentual_gastos_nao_essenciais = (
            total_gastos_nao_essenciais * 100 / total
        )
